...
function loadUsersTable() {
  ...
  if (filterVal === 'super_admin') {
    filteredAccounts = accounts.filter(acc => acc.role === 'super_admin');
  } else if (filterVal === 'student') {
    filteredAccounts = accounts.filter(acc => acc.role === 'student');
  }
...
// --- 10.6 Họa hình Màn hình vẽ lớn & Thao tác Canvas ---
let activeDrawingState = {
  canvas: null,
  ctx: null,
  isDrawing: false,
  tool: 'line', // line, erase, tmtc
  lineType: 'solid', // solid, dashed, arrow
  startX: 0,
  startY: 0,
  elements: [],
  bgImage: null,
...
function openGraphicDrawingWorkspace(qId) {
  const q = personalQuestions.find(item => item.id === qId);
  if (!q) return;

  activeDrawingState.qId = qId;
  activeDrawingState.elements = [];
  activeDrawingState.tool = q.tools.line ? 'line' : (q.tools.erase ? 'erase' : 'tmtc');
  activeDrawingState.lineType = 'solid';

  const overlay = document.createElement('div');
  overlay.id = 'graphic-workspace-overlay';
  ...
  const lineEnabled = q.tools && q.tools.line;
  const eraseEnabled = q.tools && q.tools.erase;
  const tmtcEnabled = q.tools && q.tools.tmtc;

  overlay.innerHTML = `
    <div class="d-flex justify-content-between align-items-center px-4 py-3 border-bottom border-secondary bg-dark-glass">
      <div>
        <h5 class="m-0 fw-bold text-info"><i class="bi bi-brush me-2"></i>Họa hình: ${q.question}</h5>
      </div>
      <div class="d-flex gap-2">
        <button class="btn btn-outline-warning btn-sm rounded-pill px-3 fw-bold" onclick="clearGraphicDrawingCanvas()">
          <i class="bi bi-trash3 me-1"></i>Xóa nét vẽ
        </button>
        <button class="btn btn-danger btn-sm rounded-pill px-3 fw-bold" onclick="closeGraphicDrawingWorkspace()">
          <i class="bi bi-x-circle me-1"></i>Thoát màn hình vẽ
        </button>
      </div>
    </div>
    
    <div class="d-flex flex-grow-1 position-relative" style="overflow: hidden;">
      <div id="graphic-drawing-toolbar" style="
        position: absolute;
        left: 20px;
        top: 20px;
        display: flex;
      ...
      ">
        ${lineEnabled ? `
          <button class="btn btn-info text-dark rounded-3 px-3 py-2 fw-bold d-flex flex-column align-items-center gap-1" id="btn-draw-tool-line" onclick="setGraphicDrawingTool('line')">
            <i class="bi bi-slash-lg fs-5"></i>
            <span style="font-size: 0.65rem;">Line</span>
          </button>
          
          <div id="graphic-line-types-panel" class="d-flex flex-column gap-1 bg-black bg-opacity-40 p-2 rounded-3 border border-secondary mt-1">
            <button class="btn btn-sm btn-dark text-light py-1 text-start" style="font-size: 0.75rem;" id="btn-line-type-solid" onclick="setGraphicLineType('solid')">
              <i class="bi bi-dash-lg me-1 text-info"></i> Nét liền
            </button>
            <button class="btn btn-sm btn-dark text-light py-1 text-start" style="font-size: 0.75rem;" id="btn-line-type-dashed" onclick="setGraphicLineType('dashed')">
              <i class="bi bi-segmented-nav me-1"></i> Nét đứt
            </button>
            <button class="btn btn-sm btn-dark text-light py-1 text-start" style="font-size: 0.75rem;" id="btn-line-type-arrow" onclick="setGraphicLineType('arrow')">
              <i class="bi bi-arrow-up-right me-1"></i> Mũi tên
            </button>
          </div>
        ` : ''}
        
        ${eraseEnabled ? `
          <button class="btn btn-outline-light rounded-3 px-3 py-2 fw-bold d-flex flex-column align-items-center gap-1" id="btn-draw-tool-erase" onclick="setGraphicDrawingTool('erase')">
            <i class="bi bi-eraser-fill fs-5"></i>
            <span style="font-size: 0.65rem;">Erase</span>
          </button>
        ` : ''}
        
        ${tmtcEnabled ? `
          <button class="btn btn-outline-light rounded-3 px-3 py-2 fw-bold d-flex flex-column align-items-center gap-1" id="btn-draw-tool-tmtc" onclick="setGraphicDrawingTool('tmtc')">
            <span style="border: 2px solid #fff; border-radius: 50%; width: 22px; height: 22px; display: flex; align-items: center; justify-content: center; font-size: 0.5rem; font-weight: 900;">TM</span>
            <span style="font-size: 0.65rem;">TMTC</span>
          </button>
        ` : ''}
      </div>
      
      <div class="w-100 h-100 d-flex align-items-center justify-content-center" id="graphic-canvas-wrapper" style="overflow: auto; background: #0b0f19; padding: 20px;">
        <div class="position-relative" id="graphic-workspace-inner" style="box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.75);">
          <img id="graphic-drawing-bg" src="${q.image}" style="pointer-events: none; display: block; max-width: 90vw; max-height: 80vh; object-fit: contain;">
          <canvas id="graphic-drawing-canvas" style="position: absolute; top: 0; left: 0; cursor: crosshair;"></canvas>
        </div>
      </div>
    </div>
  `;
  ...
  const img = document.getElementById('graphic-drawing-bg');
  const canvas = document.getElementById('graphic-drawing-canvas');

  const setupCanvas = () => {
    canvas.width = img.clientWidth;
    canvas.height = img.clientHeight;
    
    activeDrawingState.canvas = canvas;
    activeDrawingState.ctx = canvas.getContext('2d');
    activeDrawingState.bgImage = img;
    
    redrawGraphicCanvas();
    
    canvas.addEventListener('mousedown', startDrawingGraphic);
    canvas.addEventListener('mousemove', drawGraphic);
    canvas.addEventListener('mouseup', stopDrawingGraphic);
    canvas.addEventListener('mouseleave', stopDrawingGraphic);
    
    canvas.addEventListener('touchstart', startDrawingGraphicTouch, { passive: false });
    canvas.addEventListener('touchmove', drawGraphicTouch, { passive: false });
    canvas.addEventListener('touchend', stopDrawingGraphicTouch);
...
function closeGraphicDrawingWorkspace() {
  const overlay = document.getElementById('graphic-workspace-overlay');
  if (overlay) {
    overlay.remove();
  }
  activeDrawingState.canvas = null;
  activeDrawingState.ctx = null;
}

function setGraphicDrawingTool(tool) {
  activeDrawingState.tool = tool;
  updateToolbarUI();
}

function setGraphicLineType(type) {
  activeDrawingState.lineType = type;
  activeDrawingState.tool = 'line';
  updateToolbarUI();
}

function updateToolbarUI() {
  const btnLine = document.getElementById('btn-draw-tool-line');
  const btnErase = document.getElementById('btn-draw-tool-erase');
  const btnTmtc = document.getElementById('btn-draw-tool-tmtc');
  const subPanel = document.getElementById('graphic-line-types-panel');
  
  if (btnLine) {
    if (activeDrawingState.tool === 'line') {
      btnLine.className = 'btn btn-info text-dark rounded-3 px-3 py-2 fw-bold d-flex flex-column align-items-center gap-1';
      if (subPanel) subPanel.classList.remove('d-none');
    } else {
      btnLine.className = 'btn btn-outline-light rounded-3 px-3 py-2 fw-bold d-flex flex-column align-items-center gap-1';
      if (subPanel) subPanel.classList.add('d-none');
    }
  }
  
  if (btnErase) {
    if (activeDrawingState.tool === 'erase') {
      btnErase.className = 'btn btn-info text-dark rounded-3 px-3 py-2 fw-bold d-flex flex-column align-items-center gap-1';
    } else {
      btnErase.className = 'btn btn-outline-light rounded-3 px-3 py-2 fw-bold d-flex flex-column align-items-center gap-1';
    }
  }
  
  if (btnTmtc) {
    if (activeDrawingState.tool === 'tmtc') {
      btnTmtc.className = 'btn btn-info text-dark rounded-3 px-3 py-2 fw-bold d-flex flex-column align-items-center gap-1';
    } else {
      btnTmtc.className = 'btn btn-outline-light rounded-3 px-3 py-2 fw-bold d-flex flex-column align-items-center gap-1';
    }
  }

  ['solid', 'dashed', 'arrow'].forEach(t => {
    const el = document.getElementById(`btn-line-type-${t}`);
    if (el) {
      if (activeDrawingState.lineType === t && activeDrawingState.tool === 'line') {
        el.className = 'btn btn-sm btn-info text-dark py-1 text-start w-100 fw-bold';
      } else {
        el.className = 'btn btn-sm btn-dark text-light py-1 text-start w-100';
      }
    }
  });
}

function startDrawingGraphic(e) {
  const rect = activeDrawingState.canvas.getBoundingClientRect();
  const x = e.clientX - rect.left;
  const y = e.clientY - rect.top;
  
  activeDrawingState.isDrawing = true;
  activeDrawingState.startX = x;
  activeDrawingState.startY = y;

  if (activeDrawingState.tool === 'tmtc') {
    activeDrawingState.elements.push({
      type: 'tmtc',
      x: x,
      y: y
    });
    redrawGraphicCanvas();
    activeDrawingState.isDrawing = false;
  }
}

function drawGraphic(e) {
  if (!activeDrawingState.isDrawing) return;
  ...
  if (activeDrawingState.tool === 'line') {
    redrawGraphicCanvas();
    drawTempLine(activeDrawingState.startX, activeDrawingState.startY, x, y, activeDrawingState.lineType);
  } else if (activeDrawingState.tool === 'erase') {
    eraseElementsNear(x, y);
    redrawGraphicCanvas();
  }
}

function stopDrawingGraphic(e) {
  if (!activeDrawingState.isDrawing) return;
  ...
  if (activeDrawingState.tool === 'line') {
    activeDrawingState.elements.push({
      type: 'line',
      lineType: activeDrawingState.lineType,
      x1: activeDrawingState.startX,
      y1: activeDrawingState.startY,
      x2: x,
      y2: y
    });
    redrawGraphicCanvas();
...
function startDrawingGraphicTouch(e) {
  e.preventDefault();
  const t = e.touches[0];
  startDrawingGraphic(t);
}
function drawGraphicTouch(e) {
  e.preventDefault();
  const t = e.touches[0];
  drawGraphic(t);
}
function stopDrawingGraphicTouch(e) {
  stopDrawingGraphic();
}

function clearGraphicDrawingCanvas() {
  if (confirm("Bạn có muốn xóa toàn bộ nét vẽ hiện tại?")) {
    activeDrawingState.elements = [];
    redrawGraphicCanvas();
  }
}

function eraseElementsNear(x, y) {
  const threshold = 15;
  activeDrawingState.elements = activeDrawingState.elements.filter(el => {
    if (el.type === 'tmtc') {
...
function drawTempLine(x1, y1, x2, y2, type) {
  const ctx = activeDrawingState.ctx;
  ctx.save();
...
function redrawGraphicCanvas() {
  const canvas = activeDrawingState.canvas;
  const ctx = activeDrawingState.ctx;
  if (!canvas || !ctx) return;

  ctx.clearRect(0, 0, canvas.width, canvas.height);

  activeDrawingState.elements.forEach(el => {
    if (el.type === 'line') {
      ctx.save();
      ...
      ctx.fillText('TMTC', el.x, el.y);
      ctx.restore();
    }
  });
}

// --- 10.7 Bình luận & Thảo luận môn học ---
